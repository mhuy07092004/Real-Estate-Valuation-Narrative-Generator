...
Required env var (local `.env` or Render): `GROQ_API_KEY` (get one at https://console.groq.com/keys). Optional: `GROQ_MODEL` (default `llama-3.3-70b-versatile`), `GROQ_BASE_URL`, `GROQ_TEMPERATURE`, `GROQ_MAX_TOKENS` — see [`backend/.env.example`](backend/.env.example).
