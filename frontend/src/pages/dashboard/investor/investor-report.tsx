...
import {
  ...
  type InvestorReportListItem,
  type InvestorReportListStatus,
...
export function InvestorReport() {
  const { data, isLoading } = useAsyncData(getInvestorReportListMockData, [])
  const [reports, setReports] = useState<InvestorReportListItem[]>([])
  ...
  function handleStatusChange(id: string, status: InvestorReportListStatus) {
    setReports((current) =>
    ...
    <div className="flex min-h-full flex-col">
      <div className="flex flex-1 flex-col gap-6 p-4 sm:gap-7 sm:p-6 lg:p-8">
        ...
        {isLoading && reports.length === 0 ? (
          ...
          <p className="text-sm text-relaive-gray">No investment reports yet.</p>
        ) : (
