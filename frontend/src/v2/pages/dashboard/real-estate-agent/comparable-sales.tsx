...
import { ComparableSalesView } from '../../../features/dashboard/components/comparable-sales/comparable-sales-view'

// New standalone page (net-new route, no v1 counterpart) — figma: ComparableSalesPage.tsx
// (read in full, all 278 lines). Shares its results rendering with the generate-report
// wizard's Comparables step via ComparableSalesView (§4.5) rather than duplicating it.
// Same real backend endpoint as the wizard (/api/appraisal/comparable-sales), keyed by
// the address typed in below. See figma-ui-migration-plan.md §9.1 / §9.4 step 5 / §9.7 B.
...
function SubjectPropertyCard({ address }: { address: string }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Could not save property. Please try again.')
      setSaveState('idle')
    }
  }
  ...
  return (
    <div className="flex flex-col">
      <header className="font-sans px-4 pt-4 sm:px-6 sm:pt-6 lg:px-8 lg:pt-8">
        <h1 className="text-2xl font-semibold tracking-tight text-[#1C2A38] sm:text-[28px]">Comparable Sales</h1>
        <p className="mt-1 text-sm text-[#1C2A3880] sm:text-base">
          Search recent comparable sales for any address
        </p>
      </header>
      ...
      <div className="flex flex-col gap-5 p-4 sm:gap-6 sm:p-6 lg:p-8">
        ...
        {!searchedAddress ? (
          <div className="rounded-2xl border border-black/5 bg-white py-12 text-center">
            <p className="text-sm font-medium text-relaive-navy">Enter an address to get started</p>
            <p className="mt-1 text-sm text-relaive-gray">We'll pull the closest comparable sales.</p>
          </div>
        ) : isLoading ? (
          ...
          <>
            <SubjectPropertyCard address={searchedAddress} />
            <div>
              <h3 className="mb-4 text-base font-semibold text-relaive-navy">Similar Sales Nearby</h3>
